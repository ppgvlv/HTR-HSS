...
def main():
    ...
    model = create_model(
        ...
        use_asymmetric=not args.no_aniso,      # A0
        use_csp=not args.no_csp,               # A1
        enable_mamba=not args.no_mamba,        # A3
        enable_attn=not args.no_attn,          # A2
    )
    model = model.cuda()
    ...
    for nb_iter in range(1, args.total_iter):
        ...
        if nb_iter % args.print_iter == 0:
            ...
            logger.info(
                ...
                f'training loss : {train_loss_avg:0.5f} \t '
            )
        ...
        if nb_iter % args.eval_iter == 0:
            ...
            with torch.no_grad():
                ...
                logger.info(
                    f'Val. loss : {val_loss:0.3f} \t CER : {val_cer:0.4f} \t '
                    f'WER : {val_wer:0.4f} \t '
                )
